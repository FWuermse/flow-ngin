...
| Overlap / z-order | Handled automatically (GPU renders front-to-back) so that only the topmost ID survives |
...
| Render correctness | Parent must trust that children render with the correct pick ID |
