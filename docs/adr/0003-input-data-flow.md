...
The UI library needs interactive input widgets (text fields, checkboxes, sliders) that produce values. These values must reach the application in a way that is composable.
...
| Composability | Each widget needs its own event variant |
